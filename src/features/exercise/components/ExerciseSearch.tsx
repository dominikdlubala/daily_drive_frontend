...
interface ExerciseSearchProps2 {
    onExerciseSelect: (exercise: ExerciseDefinition) => void;
}

export function ExerciseSearch2({ onExerciseSelect }: ExerciseSearchProps2) {
  const dispatch = useAppDispatch(); 
  ...
  const handleSubmit = (exercise: ExerciseDefinition) => {
    onExerciseSelect(exercise); 
    setShowResults(false);
    setSearchTerm(""); 
    setSearchResults([]); 
  ...
  return (
    <div className="exercise_search">
      <div className="exercise_search-input">
        <input
          type="text"
          placeholder="Wyszukaj ćwiczenie"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
        />
        <button className="exercise_search-button" onClick={handleSearch} disabled={isLoading}>
          { !isLoading ? 'Szukaj' : 'Szukam...'}
        </button>
      </div>
      {error && <span className="exercise_search-error">{error}</span> }

      {searchResults?.length > 0 && showResults && (
        <ul className="exercise_search-results" ref={resultsRef}>
          {searchResults.map((exercise, index) => (
            <li className="exercise_search-result--item" key={index} 
              onClick={() => handleSubmit(exercise)}
            >
              {exercise.name}
            </li>
      ...
      <div className="exercise_search-create--wrapper">
        <span>Jeśli Twojego ćwiczenia nie ma na liście: </span>
        <button 
          type="button"
          className="button exercise_search--button exercise-search__button--add"
          onClick={() => dispatch(openModal({ type: 'CREATE_EXERCISE' }))}  
        >Dodaj nowe +</button>
      </div>
    </div>
